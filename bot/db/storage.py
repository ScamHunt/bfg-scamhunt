...
async def upload_img_to_supabase(image: PhotoSize, reporter_id: int, report_id: int):
...
def get_img_from_supabase(reporter_id, report_id) -> bytes:
    ...
    byte_img = supabase.storage.from_("screenshots").download(
        f"/{reporter_id}/report_{report_id}"
    )
    return byte_img
